...
6. [文档体系建设](#文档体系建设)
7. [测试与验证策略](#测试与验证策略)
8. [减少人工干预的实装指南](#减少人工干预的实装指南)
9. [依赖管理与安全](#依赖管理与安全)
10. [多人协作标准](#多人协作标准)
11. [预测报告与风险管理](#预测报告与风险管理)
12. [时间戳与日志规范](#时间戳与日志规范)
13. [发布与维护流程](#发布与维护流程)
14. [检查清单](#检查清单)
...
6. **自动验证与智能告警**（减少人工干预的核心原则）
   - **目标**：将原本需要人工确认的操作转换为自动验证，通过日志和告警系统汇报问题
   - **验证方法**：采用 Read-Write-Verify 三步法，在写入后立即读回检查一致性
   - **告警分级**：DEBUG/INFO（正常信息）| WARN（潜在问题）| ERROR/FATAL（需要处理的故障）
   - **日志作用**：
     - 自动记录关键操作和异常，无需手工检查日志
     - 定期生成诊断报告，自动统计成功率、失败原因、改进建议
     - 问题汇总到单一报告，避免逐文件翻查
   - **自动处理清单**：
     - ✅ 元数据提取失败：自动降级（MetadataExtractor → RawBytes → Cache）
     - ✅ 文件转换验证：写后读回检查尺寸、格式、质量是否匹配源文件
     - ✅ 元数据回写验证：验证输出文件中是否存在完整的 FullInfo
     - ✅ 文件时间戳验证：写后读回检查修改时间是否正确应用
     - ✅ 报告生成：自动生成 XLSX + 日志文件，无需手工整理
   - **示例**：
     ```
     原始流程（需人工干预）：
     用户 → 运行转换 → 检查控制台输出 → 逐一查看输出文件 → 检查是否转换成功
     
     改进流程（自动验证 + 智能告警）：
     用户 → 运行转换 → 系统自动验证每一步 → 生成汇总报告 + 诊断日志
            → 用户只需查看报告中的失败列表和改进建议
     ```
...
| 类/接口 | PascalCase | `ImageConverter`, `IValidator` | 接口带 I 前缀；库来源标记在文档表中 |
...
| 库来源标记 | XML 注释 | `/// 使用 SixLabors.ImageSharp` | 官方/第三方/自写 |
...
## TODO 管理规范（从想法到代码实现的优化过程）

项目开发的每个阶段都应该通过 TODO 列表来规划和追踪，确保**想法清晰**、**实现完整**、**过程可追溯**。

### TODO 的三重角色

TODO 不仅是任务清单，更是：
1. **规划文档**：将模糊的想法转化为具体的、可验证的工作项
2. **代码注释**：每个 TODO 背后的设计思考融入代码中
3. **变更总结**：本轮迭代完成的内容与验证标准
...
ID    | 标题（简明，3-7 字）
------|----
描述  | 详细说明该工作项的目标、方法、验收标准
      | 包含关键步骤、技术选型、预期输出、可能的障碍
```

**示例**：

```
1 | 验证 MetadataWriter PNG 实现
---|
描述 | 运行单元测试验证 MetadataWriter.cs 的 PNG 二进制写入逻辑。
   | 需测试：tEXt 块长度、大端序、CRC 计算、块插入、IEND 定位等关键点。
   | 若失败则调试修复 PNG 块构造逻辑。
   | 验收标准：
   |   ✓ tEXt 块长度正确（大端序 4 字节）
...
2 | 开发模式测试 PNG 转换端到端流程
---|
描述 | 在测试目录（C:\Users\10374\Desktop\test）运行完整转换流程，
   | 验证从读取源文件 → 转换 → 写入元数据 → 验证的完整链路。
   | 检查输出：
   |   - PNG 文件转换成功，尺寸合理，色彩空间正确
   |   - metadata-writer.log 显示 Written=True/False, Verified=True/False
...
**状态转移规则**：
- 仅在开始工作前的瞬间，标记为 `in-progress`
- 完成工作后**立即**标记为 `completed`（不要等待验证或部署）
- 避免长期停留在 `in-progress` 状态（表示工作卡顿）
...
/// 在 MetadataWriter 完成后测试读-写-验证链路
/// </summary>
...
{
    // TODO #1: 验证 tEXt 块构造逻辑
    // BuildPngTextChunk() 函数中已实现大端序长度 + CRC 计算
    // 见下方 WritePngTextChunk 实现
...
private static bool WritePngTextChunk(string pngPath, string keyword, string text)
{
    ...
    // 2. 定位 IEND 块（0x49454E44）
    var iendIndex = IndexOfBytes(bytes, new byte[] { 0x49, 0x45, 0x4E, 0x44 });
    if (iendIndex < 0) throw new InvalidOperationException("PNG IEND 块未找到");
...
private static byte[] BuildPngTextChunk(string keyword, string text)
{
    // TODO #1: PNG tEXt 块格式
    // 块结构：
    //   [4字节] 块长度（不包括长度和 CRC 字段）
    //   [4字节] 块类型 "tEXt" (0x74455874)
...
public class MetadataWriterTests
{
    ...
    public void WritePngTextChunk_ValidInput_CreatesValidTextChunk()
    {
        ...
        // 块长度应该是：keyword长度 + 1(null分隔) + text长度
...
```
发布版本 v1.1.0 前：
- [ ] 所有 TODO 状态为 completed（未完成的应进行风险评估）
- [ ] 所有 TODO 的验收标准已在测试中验证
- [ ] 所有 TODO 的代码已通过 Code Review
- [ ] CHANGELOG.md 中总结了本版本的 TODO 列表
- [ ] 相关文档已更新，引用了新功能的 TODO 背景
...
- 完整 AI 元数据提取与写回 (TODO #1-#3)
  - 实现 PNG tEXt 块二进制写入（含 CRC 校验）
  - 支持元数据读写验证链路
  
- 自动验证与诊断系统 (TODO #4-#6)
  - LogAnalyzer 自动分析日志，生成诊断报告
  - 支持分级告警（DEBUG/INFO/WARN/ERROR/FATAL）

### Fixed
- 修复 JPEG/WebP 元数据读取返回空值 (TODO #7)

### Documentation
- 新增《减少人工干预实装指南》(TODO #4-#6 参考)
- 新增《TODO 管理规范》
```
...
**推荐工具**：
- **本项目**：VS Code 集成的 TODO List 扩展（方便快速查看）
- **备选**：GitHub Issues（用于多人协作，便于追踪和评论）
- **团队协作**：Jira、Azure DevOps（大型团队）

**日常工作流**：

```bash
# 开发者开始一天的工作
1. 查看 TODO 列表（`manage_todo_list read`）
2. 选择优先级最高的未完成 TODO
3. 标记为 in-progress
4. 编写代码 + 单元测试（在代码中嵌入 TODO ID 和验收标准）
5. 本地测试通过后，标记为 completed
6. 提交 PR，描述中引用 TODO ID（如 "Closes TODO #3"）
7. Code Review 时检查 TODO 的验收标准是否全部满足
8. 合并后，更新 CHANGELOG

每周审查：
- 检查是否有超过 3 天仍在 in-progress 的 TODO（表示卡顿）
- 识别 blocked 的 TODO（等待外部输入或决策）
- 重新评估优先级，确保高价值的 TODO 得到及时推进
```
...
本章详细说明如何通过自动验证、日志告警、诊断报告等手段，**将需要人工确认的操作自动化**。

### 核心策略

#### 1. 自动验证（Automated Verification）

**原则**：每个关键操作的写入后立即进行读回验证，无需人工检查

**实装场景**：

| 操作 | 写入内容 | 验证方法 | 失败处理 |
|-----|--------|--------|--------|
| 图片格式转换 | 新的 PNG/JPG/WebP | 读回尺寸/色彩空间，比对源文件 | 记录 ERROR，列入失败清单 |
| 元数据提取 | AI Prompt、Model、Steps 等 | 提取后验证字段完整性 | 自动降级到 RawBytes |
| 元数据写入 | FullInfo 写回 PNG tEXt 块 | 读回并用 substring 比对 | 记录 WARN，继续处理 |
| 文件时间戳 | 修改时间、创建时间 | 写后读回，对比原始值 | 平台不支持时标记为 INFO |
| 报告生成 | XLSX 文件 | 验证文件大小、行数是否匹配记录数 | 重新生成或报告异常 |

**代码示例**：
```csharp
/// <summary>
/// 将元数据写入输出文件，并自动验证写入成功。
/// </summary>
...
#### 2. 智能告警系统（Intelligent Alert System）

**原则**：问题自动分类、汇总、优先排序，用户只需查看报告中的异常

**告警级别**：

```
DEBUG   → 开发调试信息（一般不在生产日志中输出）
INFO    → 正常进度信息（转换成功、处理文件数）
WARN    → 潜在问题但可继续（元数据截断、兼容性提示、验证不通过但非关键）
ERROR   → 单次操作失败（转换失败、权限错误、文件损坏）
FATAL   → 不可恢复错误（资源耗尽、核心依赖失败）
```
...
public class DiagnosisReport
{
    ...
    public bool HasAlarm => 
        (double)FailedConversions / TotalFiles > 0.1 ||  // >10% 转换失败
        (double)MetadataWriteFailures / TotalFiles > 0.05;  // >5% 元数据写入失败
    
    public List<string> Suggestions { get; set; }  // 改进建议列表
}
```

**日志输出示例**：
```
[2025-11-16 14:30:22] [INFO] [ConversionService] 开始处理目录: C:\images
[2025-11-16 14:30:23] [INFO] [FileScanner] 发现 150 个图片文件
[2025-11-16 14:30:25] [INFO] [ImageConverter] 转换成功: image_001.png → image_001.jpg
[2025-11-16 14:30:26] [WARN] [AIMetadataExtractor] 元数据提取使用备用方案 (RawBytes): image_002.png
[2025-11-16 14:30:27] [ERROR] [ImageConverter] 转换失败: image_003.gif (格式不支持)
[2025-11-16 14:30:28] [WARN] [MetadataWriter] 元数据写入验证失败: image_005.jpg (部分字段缺失)
...
[2025-11-16 14:30:45] [INFO] [ReportService] 报告已生成: conversion-report-20251116-143045.xlsx

=============== 诊断摘要 ===============
总处理: 150 | 成功: 148 | 失败: 2 | 成功率: 98.67%
元数据提取: 145 | 完整信息: 138 | 备用方案: 7
元数据写入: 146 | 验证通过: 143 | 验证失败: 3

⚠️ WARN 警告:
  - image_002.png: RawBytes 备用方案
  - image_005.jpg: 元数据验证失败 (缺失 steps 字段)
  - image_012.png: 文件时间戳无法设置 (Linux 不支持)

❌ ERROR 失败:
  - image_003.gif: 格式不支持 (GIF animated)
  - image_042.jpg: 文件损坏 (truncated JPEG)

💡 建议:
  1. 检查 image_003.gif 和 image_042.jpg，考虑手动转换或替换源文件
  2. 对于 RawBytes 备用方案的文件，考虑使用专门工具提取完整元数据
  3. Linux 环境下无法设置创建时间，但修改时间已应用

报告位置: C:\Users\10374\Desktop\test\diagnosis-report-20251116-143045.log
========================================
```

#### 3. 日志系统（Logging System）

**原则**：关键操作自动记录到文件，生成可检索的诊断证据

**日志文件规划**：

```
工作目录/
├── metadata-extraction-log.log      (元数据提取日志，仅记 ALARM 和 RawBytes)
├── metadata-writer.log               (元数据写入验证结果)
└── sourceFolder/
    ├── conversion-report-20251116-143022.xlsx  (转换报告，包含所有字段)
    ├── conversion.log                          (追加式日志，累积所有历史)
    └── diagnosis-report-20251116-143045.log    (诊断报告，统计+建议)
```
    ...
    string method)
{
    ...
    {
        (true, true) => "✓ Written + Verified",
        (true, false) => "⚠ Written, Verification Failed",
        (false, _) => "✗ Write Failed"
    };
...
#### 4. 诊断报告生成（Diagnostic Report Generation）

**原则**：自动分析日志，生成人类可读的诊断报告，无需人工翻查日志

**报告内容**：

```
诊断报告 (diagnosis-report-{timestamp}.log)
├── 头部信息
│   ├── 生成时间
│   ├── 源目录
│   ├── 处理耗时
...
├── 统计数据
│   ├── 总文件数、成功数、失败数、成功率
│   ├── 元数据提取：完整、RawBytes、缓存命中率
│   └── 元数据写入：成功、验证失败、未尝试
├── 警告阈值检查
│   ├── 转换失败率 > 10%？→ 标记为 ALARM
│   ├── RawBytes 比例 > 5%？→ 建议改进元数据源
│   └── 验证失败率 > 5%？→ 建议检查输出格式兼容性
├── 失败详情（仅列失败的，最多前 20 条）
│   ├── 文件名
│   ├── 失败原因
    ...
    TimeSpan duration)
{
    ...
    sb.AppendLine($"处理耗时: {duration.TotalSeconds:F1} 秒");
    sb.AppendLine();
    
    // 统计
    sb.AppendLine("【统计数据】");
    sb.AppendLine($"总计: {rows.Count} | 成功: {successCount} | 失败: {failureCount} | 成功率: {successRate:F1}%");
    
    // 警告
    if (failureCount > rows.Count * 0.1)
        sb.AppendLine("⚠️ 警告：转换失败率 > 10%");
    if (rows.Count(r => r.FullAIMetadataExtractionMethod == "RawBytes") > rows.Count * 0.05)
        sb.AppendLine("⚠️ 警告：RawBytes 备用方案占比 > 5%");
    
    // 失败详情
    if (failureCount > 0)
    {
        sb.AppendLine();
        sb.AppendLine("【失败详情】(前 5 条)");
        foreach (var row in rows.Where(r => !r.Success).Take(5))
        {
            sb.AppendLine($"  ❌ {Path.GetFileName(row.SourcePath)}: {row.ErrorMessage}");
        }
    }
    ...
    sb.AppendLine("【改进建议】");
    if (failureCount > 0)
        sb.AppendLine("  1. 检查失败文件列表，考虑手动修复或排除");
    if (rows.Count(r => !r.MetadataWritten) > 0)
        sb.AppendLine("  2. 部分文件元数据写入失败，检查输出格式是否支持该元数据");
...
#### 5. 控制台摘要显示（Console Summary Display）

**原则**：在控制台输出精简的摘要，让用户一眼看清结果，无需打开文件
...
⚠️ 警告信息:
   - image_002.png: 使用 RawBytes 备用方案
   - image_005.jpg: 元数据验证失败 (缺失 steps 字段)
   (共 3 条警告，详见: diagnosis-report-20251116-143045.log)
...
📁 输出位置:
   转换报告: C:\Users\10374\Desktop\test\conversion-report-20251116-143045.xlsx
   诊断日志: C:\Users\10374\Desktop\test\diagnosis-report-20251116-143045.log
   
💡 快速诊断: 98.67% 成功率，无重大警告。若需详细信息，请查看诊断日志。
...
public static void DisplaySummary(List<ConversionReportRow> rows, TimeSpan duration)
{
    ...
    Console.WriteLine("\n✅ 转换完成！\n");
    Console.WriteLine("📊 处理统计:");
    Console.WriteLine($"   总计: {rows.Count} 个文件");
    Console.WriteLine($"   ✓ 成功: {successCount} 个 ({(double)successCount/rows.Count*100:F2}%)");
    Console.WriteLine($"   ✗ 失败: {failureCount} 个 ({(double)failureCount/rows.Count*100:F2}%)");
    Console.WriteLine($"   耗时: {duration.TotalSeconds:F1} 秒\n");
    ...
    Console.WriteLine("📋 元数据统计:");
    Console.WriteLine($"   ✓ 提取: {withMetadata} 个");
    Console.WriteLine($"   ✓ 已写入: {metadataWritten} 个");
    Console.WriteLine($"   ✓ 已验证: {metadataVerified} 个\n");
    
    // 告警与建议
    if (failureCount > rows.Count * 0.1)
    {
        Console.ForegroundColor = ConsoleColor.Yellow;
        Console.WriteLine("⚠️ 警告：转换成功率低于 90%");
        Console.ResetColor();
    }
...
#### 6. 自动降级与容错（Auto-Degradation & Graceful Failures）

**原则**：关键操作失败时自动降级，而非中断流程

**降级策略**：

| 失败点 | 降级方案 | 继续处理 |
|--------|--------|--------|
| 元数据提取（MetadataExtractor 库失败） | 自动尝试 RawBytes 方案 | ✅ 是 |
| 元数据提取（RawBytes 也失败） | 标记为空，继续转换 | ✅ 是 |
| 元数据写入（PNG tEXt 块构造失败） | 记录 ERROR，不中断转换 | ✅ 是 |
| 文件时间戳设置（Linux 不支持） | 标记为 INFO 而非 ERROR | ✅ 是 |
| 转换验证失败 | 标记为 WARN，保留输出文件 | ✅ 是 |
...
public static AIMetadata? ReadAIMetadata(string imagePath)
{
    ...
    {
        LogExtraction(imagePath, "MetadataExtractor", $"失败: {ex.Message}");
    }
    ...
    {
        LogExtraction(imagePath, "RawBytes", $"失败: {ex.Message}");
    }
    
    // 降级方案 2：返回空元数据，继续处理
    LogExtraction(imagePath, "None", "无法提取元数据");
...
- [ ] **自动验证**：所有关键写入操作都有对应的读回验证
- [ ] **告警分级**：DEBUG/INFO/WARN/ERROR/FATAL 级别清晰划分
- [ ] **日志记录**：关键操作、异常、验证结果都有日志记录
- [ ] **诊断报告**：自动生成统计、警告、建议的诊断报告
- [ ] **控制台摘要**：在程序退出前输出精简摘要，包括成功率、失败原因、文件位置
- [ ] **自动降级**：关键路径有备选方案，避免因一个模块失败导致整体中断
- [ ] **异常捕获**：所有异常都被记录并转化为告警或诊断信息，不被吞咽
- [ ] **文档链接**：诊断报告中包含指向对应工作指南的链接，便于快速查阅
...
**核心原则**：**第三方维护的成熟库优于自写实现**

在同等功能下，应优先选择：
1. **第一优先**：官方库（System.* 命名空间）
2. **第二优先**：第三方成熟库（GitHub Star > 1K，社区活跃，文档完整）
3. **第三优先**：自写实现（仅在无合适第三方库或有特殊需求时）

**理由**：
- 第三方库由专业团队持续维护，质量更有保障
- 已有大量使用场景，Bug 修复更及时
- 开箱即用（Out-of-the-box），无需重复造轮子
- 降低技术债积累风险
...
| 需求 | 官方方案 | 第三方方案 | 选择 | 原因 |
|-----|--------|---------|------|------|
| 读取 PNG 图片信息 | 无（需手写二进制解析） | SixLabors.ImageSharp | ✅ 第三方 | 开箱即用，完整功能 |
| 图片格式转换 | 无 | SixLabors.ImageSharp | ✅ 第三方 | 支持多种格式，质量优秀 |
| 读取 Excel 文件 | 无 | ClosedXML | ✅ 第三方 | 功能完整，文档丰富 |
| 文件 I/O | System.IO | (无需第三方) | ✅ 官方 | 官方库已足够 |
| JSON 序列化 | System.Text.Json | Newtonsoft.Json | ✅ 官方/第三方 | 两者都可，官方内置更简洁 |
...
为了便于区分和维护，所有函数和方法应按使用的库进行分类标记：

| 分类 | 说明 | 示例 | 维护责任 |
|-----|------|------|--------|
| **官方** | .NET 官方库（System.* 命名空间） | System.IO, System.Linq | Microsoft |
| **第三方** | 由社区/公司维护的开源库 | SixLabors.ImageSharp, ClosedXML | 库的维护者 |
| **自写** | 项目内自行实现的功能 | ConversionService, ValidationService | 本项目团队 |
